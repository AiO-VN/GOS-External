...
local UPDATERsuccess, UPDATERversion = LocalCore:AutoUpdate({
    ...
    scriptPath = COMMON_PATH .. UPDATERScriptName .. ".lua",
    scriptUrl = "https://raw.githubusercontent.com/gamsteron/GOS-External/master/Common/" .. UPDATERScriptName .. ".lua",
    versionPath = COMMON_PATH .. UPDATERScriptName .. ".version",
    versionUrl = "https://raw.githubusercontent.com/gamsteron/GOS-External/master/Common/" .. UPDATERScriptName .. ".version"
})
if (UPDATERsuccess) then
    print(UPDATERScriptName .. " updated to version " .. UPDATERversion .. ". Please Reload with 2x F6 !");
    return;
end
...
LocalCore:OnEnemyHeroLoad(function(hero)
    if (hero.charName == "Yasuo" and hero.isEnemy) then
        IsYasuo = true;
    end
end)
...
Menu:MenuElement({name = tostring(UPDATERVersion), type = _G.SPACE, id = "Version"});
...
function GetGamsteronPrediction(unit, args, source)
    ...
    if (preMovePath == true) then
        ...
        if LocalCore:AngleBetween(Normalized(henext, hepos), prePosData.Dir) > 10 then
            return {Hitchance = 0}
        end
    end
    --]============]
